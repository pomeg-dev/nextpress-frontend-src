...
// Environment check
const isDevelopment = !process.env.SF_DOMAIN || process.env.SF_DOMAIN!.includes(
...
async function handleConnectionError(
  ...
  if (
    ...
    error.message?.includes("Session expired or invalid") ||
  ...
  try {
    if (isDevelopment) {
      ...
      const oauth2 = new OAuth2({
        ...
        loginUrl: process.env.SF_DOMAIN || "https://login.salesforce.com",
      });
      ...
      conn = new Connection({
        oauth2,
        instanceUrl: tokenResponse.instance_url || process.env.SF_DOMAIN!,
